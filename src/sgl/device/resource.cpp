...
static void fixup_buffer_desc(BufferDesc& desc)
{
...
static rhi::BufferDesc make_rhi_buffer_desc(const BufferDesc& desc)
{
...
{
    fixup_buffer_desc(m_desc);
    rhi::BufferDesc rhi_desc = make_rhi_buffer_desc(m_desc);
...
Buffer::Buffer(ref<Device> device, NativeHandle handle, BufferDesc desc)
    : Resource(std::move(device))
    , m_desc(std::move(desc))
{
    SGL_CHECK(m_desc.data == nullptr && m_desc.data_size == 0, "Native buffers cannot have initial data.");
    fixup_buffer_desc(m_desc);
    rhi::BufferDesc rhi_desc = make_rhi_buffer_desc(m_desc);
