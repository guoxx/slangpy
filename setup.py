    ...
    def _configure_android_build(self, cmake_args: list, env: dict) -> None:
        ...
        if CMAKE_CONFIG == "Debug":
            ...
            if env["CFLAGS"] != before_cflags:
                print(f"[setup.py] Android Debug: sanitized CFLAGS: '{before_cflags}' -> '{env['CFLAGS']}'")
            if env["CXXFLAGS"] != before_cxxflags:
                print(f"[setup.py] Android Debug: sanitized CXXFLAGS: '{before_cxxflags}' -> '{env['CXXFLAGS']}'")
