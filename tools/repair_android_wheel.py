...
def strip_debug_symbols(so_file: Path) -> bool:
    ...
    try:
        subprocess.run(
            [strip_cmd, "--strip-debug", str(so_file)], check=True, capture_output=True, text=True
        )
...
def repair_wheel(wheel_path: Path, dest_dir: Path) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if static_libs:
            print(
                f"[repair_android_wheel] Found {len(static_libs)} static libraries (.a) - removing them"
            )
        ...
        if build_release_wheel:
            print(
                f"[repair_android_wheel] BUILD_RELEASE_WHEEL=1, stripping debug symbols from all libraries"
            )
            strip_count = 0
            for so_file in so_files:
                if strip_debug_symbols(so_file):
                    strip_count += 1
            print(
                f"[repair_android_wheel] Successfully stripped {strip_count}/{len(so_files)} libraries"
            )
        else:
        ...
        print(
            f"[repair_android_wheel] Successfully set RUNPATH for {success_count}/{len(so_files)} libraries"
        )

        # Repackage wheel
        dest_dir.mkdir(parents=True, exist_ok=True)
        print(f"[repair_android_wheel] Repackaging wheel to {dest_dir}")
        subprocess.run(["wheel", "pack", str(unpacked_dir), "-d", str(dest_dir)], check=True)
